...
- 通用终端 light 风格 (cli prompt 范式; 不复刻任何特定平台 trade dress)
- 等宽字体走开源链: `ui-monospace, 'Cascadia Mono', 'Source Code Pro', 'JetBrains Mono',
  'DejaVu Sans Mono', 'Liberation Mono', Consolas, monospace`; **严禁** 引用 Menlo /
  Monaco / SF Mono / Courier New 等专有字体名 (合规说明见 `OPENSOURCE_DESIGN_COMPLIANCE.md`).
  信息密度优先.
